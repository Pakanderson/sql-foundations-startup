...
-- Let's trying adding a little bit to the code that only included one wrong return
SELECT 
    *
FROM
    titles
WHERE
    title LIKE '%____ing%'
    AND
    title NOT LIKE '% ___ing%';
/* This time, we've included everything with at least 4 characters before 'ing'
but also EXCLUDED the case of whitespace and three characters before 'ing' */
